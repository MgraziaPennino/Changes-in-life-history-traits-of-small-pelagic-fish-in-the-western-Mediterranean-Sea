...
data<- read.csv("Eenc_GSA6N_GSI_decomposed_females_all.csv")
...
m<- gam(GSI_trend~s(CRTY_trend,k=4)+s(CRTX_trend,k=4)+s(SST_trend,k=4)+s(NPP_trend,k=4),family=Gamma(link="log"),data=data_GSI, method="REML")#
summary(m)
m$aic######
res <- resid(m)
plot(res)
...
#### We remove CRTY to see if the model improves
m0b<- gam(GSI_trend~s(SST_trend,k=4)+s(NPP_trend,k=4),data=data_GSI, method="REML")#
summary(m0b)
m0b$aic######
res <- resid(m0b)
plot(res)
...
data_GSI$M=c(1:135) ### we create a variable that numerate the months continnuous
...
m2<- gamm((GSI_trend)~s(CRTY_trend,k=4)+s(SST_trend,k=4)+s(NPP_trend,k=4)+s(M,k=4),
          data=data_GSI, correlation = corARMA(value=c(0.9999,-0.3935), p=2, q=0, fixed=F),method="REML")  #
