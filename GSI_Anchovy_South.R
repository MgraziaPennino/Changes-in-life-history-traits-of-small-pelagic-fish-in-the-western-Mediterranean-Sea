...
data<- read.csv("Eenc_GAS01_GSI_decomposed_females_all.csv")
...
### SST and NPP were not significant, we remove them
m0<- gam(GSI_trend~s(CRTY_trend,k=4)+s(CRTX_trend,k=4),data=data_GSI, method="REML")#
summary(m0)
m0$aic######
res <- resid(m0)
plot(res)
...
pacf(res, lag.max = 25, main = "")
layout(1) 

# High autocorrelation--> include a temporal variable to account for time

data_GSI$M=c(1:136) ### we create a variable that numerate the months continnuous

m1<- gam((GSI_trend)~s(CRTY_trend,k=4)+s(CRTX_trend,k=4)+s(M,k=4),data=data_GSI,method="REML")#
...
m1a<- gamm((GSI_trend)~s(CRTY_trend,k=4)+s(CRTX_trend,k=4),data=data_GSI,method="REML")#
summary(m1a$gam)

m1b<-gamm((GSI_trend)~s(CRTY_trend,k=4)+s(CRTX_trend,k=4)+s(M,k=4),data=data_GSI,method="REML")
summary(m1b$gam)
summary(m1b$lme)
appraise(m1b$gam)
...
library(forecast)
arma_res <- auto.arima(resid(m1a$gam),
                       stationary = FALSE,trace=TRUE, 
                       seasonal = FALSE,approximation = FALSE,stepwise = TRUE)  # Best model: ARIMA(2,0,3) 


Arima(res,order=c(3,0,1)) ### calculate the coefficients 

##################  GAMM ################

m2<- gamm((GSI_trend)~s(CRTY_trend,k=4)+s(CRTX_trend,k=4),
          data=data_GSI, correlation = corARMA(value=c(0.4906,0.5736,-0.3181,0.9314), p=3, q=1, fixed=F),method="REML")  #
