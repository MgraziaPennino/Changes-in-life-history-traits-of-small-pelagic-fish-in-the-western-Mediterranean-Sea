...
data<- read.csv("Spil_GSA6S_GSI_decomposed_female_all.csv")
...
### CRTY and SST are not significant, we remove them. 

m0<- gam(GSI_trend~s(CRTX_trend,k=4)+s(NPP_trend,k=4),data=data_GSI, method="REML")#
summary(m0)
m0$aic######
res <- resid(m0)
plot(res)
...
### Check if removing the CRTX the model improves 

m0b<- gam(GSI_trend~s(NPP_trend,k=4),data=data_GSI, method="REML")#
summary(m0b)
m0b$aic######
res <- resid(m0b)
plot(res)
...
data_GSI$M=c(1:92) ### we create a variable that numerate the months continnuous

m1<- gam((GSI_trend)~s(CRTX_trend,k=4)+s(NPP_trend,k=4)+s(M,k=4),data=data_GSI,method="REML")#
...
m1a<- gamm((GSI_trend)~s(CRTX_trend,k=4)+s(NPP_trend,k=4),data=data_GSI,method="REML")#
summary(m1a$gam)

m1b<-gamm((GSI_trend)~s(CRTX_trend,k=4)+s(NPP_trend,k=4)+s(M,k=4),data=data_GSI,method="REML")
summary(m1b$gam)
summary(m1b$lme)
appraise(m1b$gam)
...
arma_res <- auto.arima(resid(m1b$gam),
                       ...
                       seasonal = FALSE,approximation = FALSE,stepwise = TRUE)  # Best model: ARIMA(2,0,0) 


Arima(res,order=c(2,0,0)) ### calculate the coefficients 

##################  GAMM ################

m2<- gamm((GSI_trend)~s(CRTX_trend,k=4)+s(NPP_trend,k=4)+s(M,k=4),
          data=data_GSI, correlation = corARMA(value=c(0.9824,-0.2541), p=2, q=0, fixed=F),method="REML")  #
...
visreg (m1,"CRTX_trend",
        xlab="ZC trend", ylab="GSI trend",legend=F,
