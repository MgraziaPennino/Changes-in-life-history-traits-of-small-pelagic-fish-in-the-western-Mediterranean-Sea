...
##### NORTH #####
data_GSA6N<-data_GSA6N[with(data_GSA6N, order(date)),]
data_GSA6N<-transform(data_GSA6N, Time= as.numeric(date)/10)

plot(density(data_GSA6N$TALLA_CM))

data_GSA6N$Age=as.factor(data_GSA6N$Age)
...
m1<-gam(TALLA_CM~Age, data= data_GSA6N,method="REML") 
...
##  M2: Include Year ###
###################
m2<-gam(TALLA_CM~Age+s(Year, k=4), data= data_GSA6N,method="REML") 
...
m3<-gam(TALLA_CM~Age+s(Year, by= Age, bs = 'fs', k=4), data= data_GSA6N,method="REML") 
...
m4<-gam(TALLA_CM~s(Year, by= Age, bs = 'fs', k=4), data= data_GSA6N,method="REML") 
