...
data<- read.csv("Spil_GSA1_GSI_decomposed_female_all.csv")
...
### CRTX was not significant, we remove them
...
### We check if removing ST150 the model improves

m0b<- gam(GSI_trend~s(CRTY_trend,k=4)+s(NPP_trend,k=4),data=data_GSI, method="REML")#
summary(m0b)
m0b$aic######
res <- resid(m0b)
plot(res)
...
data_GSI$M=c(1:144) ### we create a variable that numerate the months continnuous
...
m2<- gamm((GSI_trend)~s(CRTY_trend,k=4)+s(SST_trend,k=4)+s(NPP_trend,k=4),
          data=data_GSI, correlation = corARMA(value=c(0.9999,-0.9999,0.2478,-0.9224), p=3, q=1, fixed=F),method="REML")  #
