...
##### CENTRAL #####
data_GSA6S<-data_GSA6S[with(data_GSA6S, order(date)),]
data_GSA6S<-transform(data_GSA6S, Time= as.numeric(date)/10)

plot(density(data_GSA6S$TALLA_CM))

data_GSA6S$Age=as.factor(data_GSA6S$Age)
...
m1<-gam(TALLA_CM~Age, data= data_GSA6S,method="REML") 
...
##  M2: Include Year ###
###################
m2<-gam(TALLA_CM~Age+s(Year, k=4), data= data_GSA6S,method="REML") 
...
m3<-gam(TALLA_CM~Age+s(Year, by= Age, bs = 'fs', k=4), data= data_GSA6S,method="REML") 
...
m4<-gam(TALLA_CM~s(Year, by= Age, bs = 'fs', k=4), data= data_GSA6S,method="REML") 
